...
- If the user mentions their scan result (risk score, risk label, or condition), explain what that means
- Suggest primary care or lifestyle steps (like regular checkups, hydration, screen breaks, protective eyewear)
- Always recommend consulting an eye specialist for serious concerns
- Keep responses concise (2-4 sentences unless more detail is needed)
- Never diagnose — only explain and advise`;


// ── Chatbot route (single, fixed) ──
app.post("/api/agents/chat", async (req, res) => {
  const msg = req.body?.message?.trim();

  if (!msg) {
    return res.json({ ok: true, reply: "Ask me a question about your eye health!" });
  }
  ...
  try {
    ...
    if (!response.ok) {
      ...
      return res.json({ ok: false, reply: "Gemini API error occurred." });
    }
...
app.get("/api/admin/stats", requireAdmin, async (req, res) => {
  try {
    const [users] = await db.query("SELECT role, status, created_at FROM users ORDER BY created_at ASC");
    const [reports] = await db.query("SELECT disease, risk_label, created_at FROM reports ORDER BY created_at ASC");
